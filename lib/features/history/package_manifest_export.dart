...
import 'package_manifest_export_io.dart'
    if (dart.library.html) 'package_manifest_export_web.dart'
    as manifest_io;
...
Future<void> sharePackageServerManifest(
  ...
  if (kIsWeb) {
    if (context.mounted) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(loc.webExportNotSupported)));
    }
    return;
  }
...
Future<void> sharePackageServerManifestWithSnackBar(
  BuildContext context,
  Package pkg,
) async {
  try {
    ...
    if (context.mounted) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(loc.exportJsonFailed('$e'))));
    }
  }
}
