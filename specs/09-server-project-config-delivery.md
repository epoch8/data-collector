...
6. **Дашборд** строится из **закэшированных** `Project` (аналог текущего `projectsProvider`, но источник — сервер, а не только `rootBundle`).
...
- Массив элементов: минимум `project_id`, `name`, `config_version`, `updated_at` (ISO 8601).
