> **Language / Язык:** [English](09-server-project-config-delivery.md) · **Русский**

# 9 — Доставка конфигов проектов с сервера (каталог, первый запуск, привязка пакетов)

Статус: **актуально** (июнь 2026). Конфиг — **Git** (`collector/config.json`), не Django DB. См. [git-backed-projects.ru.md](git-backed-projects.ru.md).
...
**Связанные документы:** [08-server-api-package-upload.ru.md](08-server-api-package-upload.ru.md) (загрузка пакета, `project_id` в URL), [07-package-payload-structure.ru.md](07-package-payload-structure.ru.md) (`project_id` в `payload.json`), [config/json-driven-collection-ui.ru.md](config/json-driven-collection-ui.ru.md) (модель `Project` и парсинг JSON на клиенте), [config/09-project-json-builder-guide.ru.md](config/09-project-json-builder-guide.ru.md) (как собирать JSON проекта).
...
| Per-project storage | `api/project_storage_config.py` — см. [project-storage-uris.ru.md](project-storage-uris.ru.md) |
