...
class StorageConfig:
    ...
    database_options: dict = field(default_factory=dict)
...
def build_postgres_database_uri(
    ...
    *,
    ...
    """Полный URI (legacy). Для новых проектов — uri без кредов + database_options."""
    dbname = postgres_db_name(project_id)
    base = f"postgresql+psycopg2://{host}:{port}/{dbname}"
    if user or password:
        return apply_database_credentials(base, {"user": user, "password": password})
    return base


def build_postgres_database_uri_base(
    project_id: str,
    *,
    host: str = "localhost",
    port: int = 55432,
...
def default_postgres_database_options(
    *,
    user: str = "collector",
    password: str = "collector",
) -> dict:
...
def decode_database_options(encrypted: str) -> dict:
    """Расшифровать JSON database_options. Пусто/ошибка -> {}."""
    raw = (encrypted or "").strip()
    if not raw:
        return {}
    try:
...
def encode_database_options(options: dict) -> str:
    """Зашифровать JSON database_options. Пустой dict -> ''."""
    clean = {k: v for k, v in (options or {}).items() if v not in (None, "")}
    if not clean:
        return ""
    from .git_credential_crypto import encrypt_private_key
...
def strip_database_credentials(uri: str) -> str:
    """Убрать user:password@ из SQLAlchemy URL (креды хранятся отдельно)."""
    value = (uri or "").strip()
...
def apply_database_credentials(uri: str, opts: dict) -> str:
    """Собрать полный database URI: host/db из uri + user/password из opts."""
    value = (uri or "").strip()
    if not value or value.startswith("sqlite"):
        return value
    user = (opts.get("user") or opts.get("username") or "").strip()
...
def mask_database_uri(uri: str) -> str:
    """URI для отображения в UI: пароль заменён на ****."""
    value = (uri or "").strip()
    if not value or value.startswith("sqlite"):
        return value
    parts = urlsplit(value)
...
def storage_options_from_post(post, existing: dict | None = None) -> dict:
    existing = existing or {}
    opts: dict = {}
    endpoint = (post.get("s3_endpoint_url") or "").strip()
    key = (post.get("s3_key") or "").strip()
    secret = (post.get("s3_secret") or "").strip()
    if endpoint:
...
def database_options_from_post(post, existing: dict | None = None) -> dict:
    existing = existing or {}
    opts: dict = {}
    user = (post.get("db_user") or "").strip()
    password = (post.get("db_password") or "").strip()
    if user:
        opts["user"] = user
    elif existing.get("user"):
        opts["user"] = existing["user"]
    if password:
        opts["password"] = password
    elif existing.get("password"):
        opts["password"] = existing["password"]
    return opts


def save_storage_from_post(
    post,
    *,
    existing_storage: dict | None = None,
    existing_db: dict | None = None,
) -> tuple[str, str, str, str]:
    """storage_uri, database_uri, storage_options_encrypted, database_options_encrypted."""
    storage_uri = normalize_storage_uri(post.get("storage_uri") or "")
    database_uri = normalize_database_uri(post.get("database_uri") or "")
    st_opts = storage_options_from_post(post, existing_storage)
    db_opts = database_options_from_post(post, existing_db)
    if db_opts:
        database_uri = strip_database_credentials(database_uri)
    return (
        storage_uri,
        database_uri,
        encode_storage_options(st_opts),
        encode_database_options(db_opts),
    )


def database_credentials_for_ui(project) -> tuple[str, bool]:
    """Логин и флаг «пароль задан» для формы редактирования."""
    opts = decode_database_options(getattr(project, "database_options_encrypted", ""))
    user = (opts.get("user") or opts.get("username") or "").strip()
    has_password = bool(opts.get("password"))
    if not user:
        parts = urlsplit((getattr(project, "database_uri", "") or "").strip())
        if parts.username:
            user = parts.username
            has_password = has_password or bool(parts.password)
...
def resolve(project) -> StorageConfig:
    ...
    db_stored = (getattr(project, "database_uri", "") or "").strip()
    ...
    storage_opts = decode_storage_options(getattr(project, "storage_options_encrypted", ""))
    db_opts = decode_database_options(getattr(project, "database_options_encrypted", ""))
    if not db_stored:
        db_uri = default_database_uri(project_id)
    elif db_opts:
        db_uri = apply_database_credentials(db_stored, db_opts)
    else:
        db_uri = db_stored
...
def ensure_project_database(project) -> tuple[bool, str]:
    """Создать Postgres-базу (если нет) и применить схему таблиц project DB."""
    return ensure_database_uri(resolve(project).database_uri)


def ensure_database_uri(database_uri: str) -> tuple[bool, str]:
    """Подготовить project DB по полному SQLAlchemy URI."""
    ...
    if not uri:
        return False, "database_uri не задан"
    if uri.startswith("sqlite"):
        pdb.get_engine_for_uri(uri)
        return True, "SQLite готов"
    if not uri.startswith("postgresql"):
        pdb.get_engine_for_uri(uri)
        return True, "Схема таблиц применена"
...
def drop_database_uri(database_uri: str) -> None:
    """Удалить project DB: SQLite-файл / Postgres DROP DATABASE / иначе drop_all."""
    import shutil
    ...
    cached = pdb._engines.pop(uri, None)
    if cached is not None:
        cached.dispose()
    pdb._initialized.discard(uri)
